...
def create_simple_template_data():
    """Gathers data from the simple template fields and generates structured content."""
    template = copy.deepcopy(DETAILED_TEMPLATE)
    
    # Simple, direct replacements
    content_map = {
        "TERM_TEXT": term_entry.get(),
        "〔ALTERNATE_FORM〕": alternative_form_entry.get(),
        "1. Primary Definition": primary_definition_entry.get(),
        "Etymology: Origin or component breakdown.": etymology_entry.get(),
        "First example sentence.": example_sentence_entry.get()
    }
    
    def replace_placeholders(node):
        if isinstance(node, str):
            return content_map.get(node, node)
        elif isinstance(node, list):
            return [replace_placeholders(item) for item in node]
        elif isinstance(node, dict):
            return {k: replace_placeholders(v) for k, v in node.items()}
        return node
    
    filled_template = replace_placeholders(template)
    
    # Use the existing clean_placeholders function to remove any unused fields.
    return clean_placeholders(filled_template)
...
def create_entry_data():
    ...
    # Define the check for required fields based on the active mode
    if simple_template_var.get():
        if not term or not primary_definition_entry.get():
            messagebox.showerror("Error", "Term and Primary Definition are required in Simple Template mode.")
            return None
    elif not term:
        messagebox.showerror("Error", "Term is required.")
        return None
    elif not (definitions_text or current_definitions is not None):
    ...
    elif simple_template_var.get():
        # User is in simple template mode
        final_definitions_content = create_simple_template_data()
    else:
...
def clear_fields():
    ...
    # Clear new template fields
    alternative_form_entry.delete(0, tk.END)
    primary_definition_entry.delete(0, tk.END)
    etymology_entry.delete(0, tk.END)
    example_sentence_entry.delete(0, tk.END)
...
def on_mode_toggle():
    """
    Handles the state change of the mode checkboxes.
    Hides/shows relevant fields and updates labels by dynamically managing grid placement.
    """
    # First, hide all definition-related UI elements to ensure a clean slate.
    definitions_label.grid_forget()
    definitions_text_box.grid_forget()
    edit_definitions_button.grid_forget()
    alternative_form_label.grid_forget()
    alternative_form_entry.grid_forget()
    primary_definition_label.grid_forget()
    primary_definition_entry.grid_forget()
    etymology_label.grid_forget()
    etymology_entry.grid_forget()
    example_sentence_label.grid_forget()
    example_sentence_entry.grid_forget()
    
    # Start the dynamic row counter
    current_row = 6

    if simple_template_var.get():
        template_mode_var.set(False)
        # Show new simple template fields
        alternative_form_label.grid(row=current_row, column=0, sticky='w', pady=2)
        alternative_form_entry.grid(row=current_row, column=1, columnspan=2, padx=5, pady=2, sticky='ew')
        alternative_form_label.config(text="〔ALTERNATE_FORM〕:")
        current_row += 1

        primary_definition_label.grid(row=current_row, column=0, sticky='w', pady=2)
        primary_definition_entry.grid(row=current_row, column=1, columnspan=2, padx=5, pady=2, sticky='ew')
        primary_definition_label.config(text="1. Primary Definition:")
        current_row += 1

        etymology_label.grid(row=current_row, column=0, sticky='w', pady=2)
        etymology_entry.grid(row=current_row, column=1, columnspan=2, padx=5, pady=2, sticky='ew')
        etymology_label.config(text="Etymology / Notes:")
        current_row += 1

        example_sentence_label.grid(row=current_row, column=0, sticky='w', pady=2)
        example_sentence_entry.grid(row=current_row, column=1, columnspan=2, padx=5, pady=2, sticky='ew')
        example_sentence_label.config(text="Example Sentence:")
        current_row += 1
        
    elif template_mode_var.get():
        simple_template_var.set(False)
        # Show old definitions box, disable it
        definitions_label.grid(row=current_row, column=0, sticky='w', pady=2)
        definitions_text_box.grid(row=current_row, column=1, padx=5, pady=2, sticky='ew')
        edit_definitions_button.grid(row=current_row, column=2, padx=5, pady=2, sticky='w')
        definitions_text_box.config(state=tk.NORMAL)
        definitions_text_box.delete("1.0", tk.END)
        ...
        # Show old definitions box, enable it
        definitions_label.grid(row=current_row, column=0, sticky='w', pady=2)
        definitions_text_box.grid(row=current_row, column=1, padx=5, pady=2, sticky='ew')
        edit_definitions_button.grid(row=current_row, column=2, padx=5, pady=2, sticky='w')
        definitions_text_box.config(state=tk.NORMAL)
    ...
    # Place the remaining fields at the end of the current layout
    sequence_label.grid(row=current_row, column=0, sticky='w', pady=2)
    sequence_entry.grid(row=current_row, column=1, columnspan=2, padx=5, pady=2, sticky='ew')
    current_row += 1

    term_tags_label.grid(row=current_row, column=0, sticky='w', pady=2)
    term_tags_entry.grid(row=current_row, column=1, columnspan=2, padx=5, pady=2, sticky='ew')
...
if __name__ == "__main__":
    ...
    # New Simple Template Fields (hidden by default)
    alternative_form_label = tk.Label(input_frame, text="Alternative Form:", anchor='w')
    alternative_form_entry = tk.Entry(input_frame, width=40)
    primary_definition_label = tk.Label(input_frame, text="Primary Definition:", anchor='w')
    primary_definition_entry = tk.Entry(input_frame, width=40)
    etymology_label = tk.Label(input_frame, text="Etymology:", anchor='w')
    etymology_entry = tk.Entry(input_frame, width=40)
    example_sentence_label = tk.Label(input_frame, text="Example Sentence:", anchor='w')
    example_sentence_entry = tk.Entry(input_frame, width=40)
    
    # Old Definitions
    definitions_label = tk.Label(input_frame, text=labels["Definitions"], anchor='w')
    ...
    # Checkbox Row (Row 5)
    simple_template_var = tk.BooleanVar()
    simple_template_check = ttk.Checkbutton(
        input_frame,
        text="Use Simple Template Mode",
        variable=simple_template_var,
        command=on_mode_toggle
    )
    simple_template_check.grid(row=5, column=0, sticky='w', padx=5, pady=(5, 0))
    
    template_mode_var = tk.BooleanVar()
    template_mode_check = ttk.Checkbutton(
        input_frame,
        text="Use Detailed Template",
        variable=template_mode_var,
        command=on_mode_toggle
    )
    template_mode_check.grid(row=5, column=1, sticky='w', padx=5, pady=(5, 0))
    
    # Remaining Entries (will be managed dynamically by on_mode_toggle)
    sequence_label = tk.Label(input_frame, text=labels["Sequence Number"], anchor='w')
    sequence_entry = tk.Entry(input_frame, width=40)
    term_tags_label = tk.Label(input_frame, text=labels["Term Tags"], anchor='w')
    term_tags_entry = tk.Entry(input_frame, width=40)
    ...
    # Initial call to set up the default UI layout
    on_mode_toggle()
